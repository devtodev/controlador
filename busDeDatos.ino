...
char evaluarTrama()
{
    ...
    {
      solicitarReenvio("" + tramaRx[0] + tramaRx[1]);
      return CRC_ERROR;
    }
...
void prepararTrama(String IP, unsigned char funcion, unsigned char data[6])
{
  ...
  avisoEnPantalla = (String("IP " + IP + " func " + funcion + " data "  + data[0]));
  enviarTrama(trama);
}
...
{
  unsigned char datos[6] = {switches, switches, 0, 0, 0, 0};
  prepararTrama(IP, SETSWITCHES, datos);
}

unsigned char StringToChar (String switches)
{
  unsigned char respuesta = 0;
  
  for (int i = 0; i < switches.length(); i++)
  {
    respuesta = (switches[i] == '1')? respuesta + 2 ^ i : respuesta ;
  }
...
void setLed(String IP, unsigned char modo)
{
...
// Set led del nodo en forma unicast para debug
void solicitarReenvio(String IP)
{
